...
- `scripts/check-rustc-hard-rules.sh`: `SEC001` syntax path 0 findings,
  ...
  findings; `SEC003` syntax path 3 findings, rustc path 4 findings; `SEC008`
  ...
  5 findings; `SEC012` syntax path 6 findings, rustc path 3 findings; `SEC013`
...
| `SEC003` had 571 findings with 97% sampled FP rate. | Partially superseded by rustc-driver increment; not fully migrated. | `polkadot-linter-rustc` now reports decode calls using resolved return/receiver types, handles associated projections such as `<T as Config>::RuntimeCall`, and requires propagated input evidence from entry parameters through locals, match patterns, `using_encoded` closures, and direct resolved local calls. `scripts/check-rustc-hard-rules.sh` proves it catches aliased, match-derived, and directly-called-private-helper runtime decodes while skipping an internal buffer and an unreachable private helper. `scripts/check-rustc-sdk-sec003.sh` proves SDK coverage on `pallet-xcm`: syntax reports 0 findings while rustc reports the associated-projection decode at line 4111. Indirect-call taint and broader SDK package coverage remain. |
| `SEC003` fired on non-recursive internal types and non-user-controlled decode calls. | Partially superseded by rustc-driver increment; still incomplete. | The rustc fixture skips `MigrationState::decode` because the resolved type is not a recursive runtime target and skips `RuntimeCall::decode` over an internal constant buffer. It also drops an unreachable private helper while retaining a private helper that receives tainted entry-point input. The SDK check filters macro-generated pallet attribute decode noise. Indirect-call and path-sensitive input taint remain future compiler-backed/dataflow work. |
