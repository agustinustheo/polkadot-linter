...
- `scripts/check-rustc-hard-rules.sh`: `SEC001` syntax path 0 findings,
  rustc path 3 findings, including a whitespace-separated
  `#[pallet :: call_index]` recognized only through rustc AST capture; `SEC002`
  syntax path 4 findings, rustc path 3
  findings; `SEC003` syntax path 11 findings, rustc path 12 findings; `SEC008`
  syntax path 16 findings, rustc path 4 findings; `SEC009` syntax path 6
  findings, rustc path 3 findings; `SEC011` syntax path 1 finding, rustc path
  9 findings; `SEC012` syntax path 6 findings, rustc path 4 findings; `SEC013`
  syntax path 0 findings, rustc path 2 findings, including a whitespace-separated
  `#[pallet :: storage]` marker recovered only through rustc AST capture; `SEC017`
  syntax path 0 findings, rustc path 2 findings, including a whitespace-separated
  ...
  that only the rustc AST capture can recover; rustc driver rule filter
  `SEC008,SEC009` emitted exactly those 7
...
| `SEC001` had 104 findings with 60% sampled FP rate. | Partially superseded by rustc-driver increment; not fully migrated. | Phase 1 added privileged-origin and bounded-input handling. Current unrestricted count is 7. The rustc path resolves aliased `Vec` inputs only for FRAME dispatchables identified through rustc's pre-expansion `#[pallet::call_index]` attribute, excludes public helpers, and recognizes an initial terminating literal length guard as a local bound; `scripts/check-rustc-sdk-smoke.sh` preserves the five validated multisig dispatchable findings. Remaining trust depends on resolved origin, parameter bounds, and input-flow evidence. |
...
| `SEC017` had 21 findings with 57% sampled FP rate. | Partially superseded by rustc-driver increment; not fully migrated. | Current unrestricted count is 12. The rustc path captures `#[pallet::event]` before macro expansion, resolves event payload aliases to `Vec`, and skips bounded wrappers and ordinary enums without borrowing a marker from a preceding item. `scripts/check-rustc-sdk-sec017.sh` proves SDK coverage on `pallet-root-offences`: syntax reports 0 findings while rustc reports the unbounded `OffenceCreated::offenders` payload at line 116. Event input-flow and weight-accounting evidence remain needed for audit grade. |
