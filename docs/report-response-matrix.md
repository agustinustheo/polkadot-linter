...
- `scripts/check-rustc-hard-rules.sh`: `SEC001` syntax path 0 findings,
  ...
  findings; `SEC003` syntax path 7 findings, rustc path 7 findings; `SEC008`
  syntax path 7 findings, rustc path 3 findings; `SEC009` syntax path 3
  ...
  3 findings; rustc driver rule filter `SEC008,SEC009` emitted exactly those 5
...
| `SEC003` fired on non-recursive internal types and non-user-controlled decode calls. | Partially superseded by rustc-driver increment; still incomplete. | The rustc fixture skips `MigrationState::decode` because the resolved type is not a recursive runtime target and skips `RuntimeCall::decode` over an internal constant buffer. It also removes taint after a local is overwritten with an internal buffer, drops an unreachable private helper, and retains a private helper that receives tainted entry-point input. The SDK check filters macro-generated pallet attribute decode noise. Indirect-call and path-sensitive input taint remain future compiler-backed/dataflow work. |
...
| `SEC008` false positives in `genesis_build`, benchmarks, `runtime-benchmarks`, and type-provably infallible conversions. | Partially superseded by rustc-driver increment; still incomplete. | Tests cover benchmark/runtime-benchmark/genesis/helper paths. The rustc fixture now covers one type-provably infallible conversion via `Result<T, Infallible>` plus locals constructed as `Ok`/`Some`; an overwrite with an unknown fallible result remains reported. Broader panic reachability and debug assertion handling still require compiler-backed control-flow and cfg evidence. |
