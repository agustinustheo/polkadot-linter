...
- all pinned SDK baseline scripts assert that `--syntax-only` emits zero public
...
registers `SEC001` through `SEC018`, and `--syntax-only` deliberately emits no
