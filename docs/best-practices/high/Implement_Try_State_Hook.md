...
> **Linter coverage — Review gate.** No shipping rule covers this. A meaningful `try_state` hook depends on each pallet's own invariants, so its presence and quality must be assessed by a human.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/high/Implement_Try_State_Hook.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
