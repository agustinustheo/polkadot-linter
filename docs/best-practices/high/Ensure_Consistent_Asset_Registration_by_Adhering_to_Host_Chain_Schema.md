...
> **Linter coverage — Review gate.** No shipping rule covers this. Correct asset registration is relative to the host chain's published `Location` schema, which the linter cannot know, so it needs human review.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/high/Ensure_Consistent_Asset_Registration_by_Adhering_to_Host_Chain_Schema.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
