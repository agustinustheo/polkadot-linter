...
> **Linter coverage — Review gate.** No shipping rule covers this. A correct `Barrier` encodes a trusted-origin policy specific to the chain, which requires human review rather than static enforcement.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/high/Properly_Setup_XCM_Barrier.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
