...
> **Linter coverage — Review gate.** No shipping rule covers this. Judging whether a `Junction` represents the intended entity depends on runtime composition and protocol intent, which needs human review.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/high/Make_Proper_Usage_of_XCM_Junctions.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
