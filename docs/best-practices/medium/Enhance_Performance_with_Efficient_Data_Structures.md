...
> **Linter coverage — Review gate.** The right data structure depends on the workload, so no shipping rule enforces this choice.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/medium/Enhance_Performance_with_Efficient_Data_Structures.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
