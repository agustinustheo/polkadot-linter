...
> **Linter coverage — Review gate.** No documentation-presence rule ships, so `Config` documentation is verified only by human review.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/medium/Provide_Pallet_Configuration_Documentation.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
