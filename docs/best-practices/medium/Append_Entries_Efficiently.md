...
> **Linter coverage — Review gate.** Converting a `try_mutate` to `try_append` can silently drop validation the mutating closure performed, so this efficiency change needs human review of the original logic. No shipping rule detects or enforces this transformation.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/medium/Append_Entries_Efficiently.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
