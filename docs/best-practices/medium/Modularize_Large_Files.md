...
> **Linter coverage — Review gate.** File-size and module-layout policy is project-specific, so no shipping rule enforces splitting large files.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/medium/Modularize_Large_Files.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
