...
> **Linter coverage — Review gate.** Migrating off the `Currency` trait depends on the SDK version and fungibles setup, so no shipping rule enforces it.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/medium/Transition_Away_from_Currency_Trait.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
