...
> **Linter coverage — Review gate.** No shipping rule enforces this: the trust policy governing which XCM assets may be teleported cannot be inferred from a call site and requires human review of the runtime's XCM configuration.
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/critical/Prioritize_Reserve_Asset_Transfer_Over_Teleport.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
