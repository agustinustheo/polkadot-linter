...
> **Linter coverage — Partial.** The linter flags unbounded storage iteration reachable from runtime entry points and unbounded `clear_prefix` calls, but it cannot prove that a chosen bound is economically safe. See [SEC011](../../rules/SEC011.md) and [SEC012](../../rules/SEC012.md).
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/critical/Avoid_Unbounded_Iteration.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
