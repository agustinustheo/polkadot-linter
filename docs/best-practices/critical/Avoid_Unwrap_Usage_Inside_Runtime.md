...
> **Linter coverage — Enforced.** The linter reports panic-inducing paths in production code — `unwrap`, `expect`, `panic!`, and `todo!` — as well as reachable `debug_assert!` calls that panic when compiled with debug assertions. See [SEC008](../../rules/SEC008.md) and [SEC002](../../rules/SEC002.md).
...
*Adapted from [Libro — Polkadot SDK Best Practices](https://libro.blockdeep.dev/critical/Avoid_Unwrap_Usage_Inside_Runtime.html) by [BlockDeep](https://github.com/blockdeep/libro), used under the Apache-2.0 license. The **Linter coverage** note above is added by the polkadot-linter project.*
