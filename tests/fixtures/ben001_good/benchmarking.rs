...
mod benchmarks {
    ...
    fn sweep_expired() {
        ...
        Pallet::<Test>::seed_expired_proposals(proposals, &caller);

        #[extrinsic_call]
        sweep_expired(RawOrigin::Signed(caller.clone()), proposals);

        verify {
            assert_last_event::<Test>(Event::ExpiredProposalsSwept { count: proposals }.into());
        }
    }
}
