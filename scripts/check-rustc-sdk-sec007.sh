...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required for scripts/check-rustc-sdk-sec007.sh" >&2
  exit 1
fi
...
BASELINE="${3:-$ROOT_DIR/benchmarks/polkadot-sdk-rustc-elections-phragmen-sec007-baseline.tsv}"
PACKAGE="pallet-elections-phragmen"
PACKAGE_DIR="$SDK_DIR/substrate/frame/elections-phragmen"
PACKAGE_FILE="substrate/frame/elections-phragmen/src/lib.rs"
TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"

mkdir -p "$OUTPUT_DIR"
SYNTAX_JSON="$OUTPUT_DIR/rustc-sdk-sec007-syntax-$TIMESTAMP.json"
RUSTC_JSON="$OUTPUT_DIR/rustc-sdk-sec007-$TIMESTAMP.json"
SUMMARY="$OUTPUT_DIR/rustc-sdk-sec007-$TIMESTAMP-summary.tsv"
...
cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC007 --syntax-only "$PACKAGE_DIR" > "$SYNTAX_JSON"

cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC007 \
...
syntax_findings="$(jq '[.[] | select(.rule_id == "SEC007")] | length' "$SYNTAX_JSON")"
...
if ! diff -u <(grep -v '^#' "$BASELINE") "$SUMMARY"; then
  echo "rustc SEC007 SDK findings differ from validated baseline" >&2
  exit 1
fi

echo "rustc SEC007 SDK findings match validated baseline"
