...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required for scripts/check-rustc-sdk-sec009.sh" >&2
  exit 1
fi
...
BASELINE="${3:-$ROOT_DIR/benchmarks/polkadot-sdk-rustc-collective-sec009-baseline.tsv}"
...
SYNTAX_JSON="$OUTPUT_DIR/rustc-sdk-sec009-syntax-$TIMESTAMP.json"
RUSTC_JSON="$OUTPUT_DIR/rustc-sdk-sec009-$TIMESTAMP.json"
RUSTC_SUMMARY_TSV="$OUTPUT_DIR/rustc-sdk-sec009-$TIMESTAMP-summary.tsv"
...
syntax_findings="$(jq '[.[] | select(.rule_id == "SEC009")] | length' "$SYNTAX_JSON")"
rustc_findings="$(wc -l < "$RUSTC_SUMMARY_TSV" | tr -d '[:space:]')"
echo "SEC009 syntax package findings: $syntax_findings"
echo "SEC009 rustc package findings: $rustc_findings"
cat "$RUSTC_SUMMARY_TSV"

test "$syntax_findings" -eq 5
test "$rustc_findings" -eq 2
test "$rustc_findings" -lt "$syntax_findings"

if ! diff -u <(sort "$BASELINE") <(sort "$RUSTC_SUMMARY_TSV"); then
  echo "rustc SEC009 SDK findings differ from validated baseline" >&2
  exit 1
fi

echo "rustc SEC009 SDK findings match validated baseline"
