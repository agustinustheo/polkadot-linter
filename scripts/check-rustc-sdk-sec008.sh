...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required for scripts/check-rustc-sdk-sec008.sh" >&2
  exit 1
fi
...
BASELINE="${3:-$ROOT_DIR/benchmarks/polkadot-sdk-rustc-multisig-sec008-baseline.tsv}"
...
cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --format json --rules SEC008 --no-rustc "$PACKAGE_DIR" > "$SYNTAX_JSON"

cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --format json --rules SEC008 --rustc-package "$PACKAGE" --rustc-lib \
...
syntax_findings="$(jq '[.[] | select(.rule_id == "SEC008")] | length' "$SYNTAX_JSON")"
rustc_findings="$(wc -l < "$SUMMARY" | tr -d '[:space:]')"
echo "SEC008 syntax package findings: $syntax_findings"
echo "SEC008 rustc package findings: $rustc_findings"
cat "$SUMMARY"
...
if ! diff -u <(sort "$BASELINE") <(sort "$SUMMARY"); then
  echo "rustc SEC008 SDK findings differ from validated baseline" >&2
  exit 1
fi

echo "rustc SEC008 SDK findings match validated baseline"
