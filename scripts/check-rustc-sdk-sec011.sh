...
cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC011 --syntax-only "$PACKAGE_DIR" > "$SYNTAX_JSON"

cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC011 --package "$PACKAGE" --lib \
  --no-default-features --driver-path "$DRIVER" \
  --toolchain nightly-2025-06-10 --target-dir "$SDK_TARGET_DIR" \
  --source-filter "$PACKAGE_FILTER" "$PACKAGE_DIR" > "$RUSTC_JSON"
