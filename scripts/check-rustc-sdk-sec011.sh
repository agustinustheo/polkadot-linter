...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required for scripts/check-rustc-sdk-sec011.sh" >&2
  exit 1
fi
...
BASELINE="${3:-$ROOT_DIR/benchmarks/polkadot-sdk-rustc-multisig-sec011-baseline.tsv}"
PACKAGE="pallet-multisig"
PACKAGE_DIR="$SDK_DIR/substrate/frame/multisig"
PACKAGE_FILTER="substrate/frame/multisig/"
PACKAGE_FILE="substrate/frame/multisig/src/migrations.rs"
TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"

mkdir -p "$OUTPUT_DIR"
SYNTAX_JSON="$OUTPUT_DIR/rustc-sdk-sec011-syntax-$TIMESTAMP.json"
RUSTC_JSON="$OUTPUT_DIR/rustc-sdk-sec011-$TIMESTAMP.json"
SUMMARY="$OUTPUT_DIR/rustc-sdk-sec011-$TIMESTAMP-summary.tsv"
...
cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC011 --no-rustc "$PACKAGE_DIR" > "$SYNTAX_JSON"

cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC011 --rustc-package "$PACKAGE" --rustc-lib \
  --rustc-no-default-features --rustc-driver "$DRIVER" \
  --rustc-toolchain nightly-2025-06-10 --rustc-target-dir "$SDK_TARGET_DIR" \
  --rustc-source-filter "$PACKAGE_FILTER" "$PACKAGE_DIR" > "$RUSTC_JSON"
...
syntax_findings="$(jq '[.[] | select(.rule_id == "SEC011")] | length' "$SYNTAX_JSON")"
rustc_findings="$(wc -l < "$SUMMARY" | tr -d '[:space:]')"
echo "SEC011 syntax package findings: $syntax_findings"
echo "SEC011 rustc package findings: $rustc_findings"
cat "$SUMMARY"
...
if ! diff -u <(sort "$BASELINE") <(sort "$SUMMARY"); then
  echo "rustc SEC011 SDK findings differ from validated baseline" >&2
  exit 1
fi

echo "rustc SEC011 SDK findings match validated baseline"
