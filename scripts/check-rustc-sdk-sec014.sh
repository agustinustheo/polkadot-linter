...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required for scripts/check-rustc-sdk-sec014.sh" >&2
  exit 1
fi
...
mkdir -p "$OUTPUT_DIR"
SYNTAX_JSON="$OUTPUT_DIR/rustc-sdk-sec014-syntax-$TIMESTAMP.json"
RUSTC_JSON="$OUTPUT_DIR/rustc-sdk-sec014-$TIMESTAMP.json"
...
cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC014 --syntax-only "$PACKAGE_DIR" > "$SYNTAX_JSON"

cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC014 \
...
syntax_findings="$(jq '[.[] | select(.rule_id == "SEC014")] | length' "$SYNTAX_JSON")"
rustc_findings="$(jq '[.[] | select(.rule_id == "SEC014")] | length' "$RUSTC_JSON")"
echo "SEC014 syntax package findings: $syntax_findings"
echo "SEC014 rustc package findings: $rustc_findings"

# The package has no `Identity` common-key candidate, preserving the zero
# baseline while the focused fixture covers documented internal indexes.
test "$syntax_findings" -eq 0
test "$rustc_findings" -eq 0

echo "rustc SEC014 SDK zero baseline matched"
