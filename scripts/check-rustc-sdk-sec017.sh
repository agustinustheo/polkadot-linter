...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required for scripts/check-rustc-sdk-sec017.sh" >&2
  exit 1
fi
...
BASELINE="${3:-$ROOT_DIR/benchmarks/polkadot-sdk-rustc-root-offences-sec017-baseline.tsv}"
PACKAGE="pallet-root-offences"
PACKAGE_DIR="$SDK_DIR/substrate/frame/root-offences"
PACKAGE_FILE="substrate/frame/root-offences/src/lib.rs"
TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"

mkdir -p "$OUTPUT_DIR"
SYNTAX_JSON="$OUTPUT_DIR/rustc-sdk-sec017-syntax-$TIMESTAMP.json"
RUSTC_JSON="$OUTPUT_DIR/rustc-sdk-sec017-$TIMESTAMP.json"
SUMMARY="$OUTPUT_DIR/rustc-sdk-sec017-$TIMESTAMP-summary.tsv"
...
cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC017 --syntax-only "$PACKAGE_DIR" > "$SYNTAX_JSON"

cargo +1.93.0 run --quiet --manifest-path "$ROOT_DIR/Cargo.toml" --bin polkadot-linter -- \
  --config "$ROOT_DIR/config/default.toml" --format json --rules SEC017 \
...
syntax_findings="$(jq '[.[] | select(.rule_id == "SEC017")] | length' "$SYNTAX_JSON")"
rustc_findings="$(wc -l < "$SUMMARY" | tr -d '[:space:]')"
echo "SEC017 syntax package findings: $syntax_findings"
echo "SEC017 rustc package findings: $rustc_findings"
cat "$SUMMARY"
...
if ! diff -u <(sort "$BASELINE") <(sort "$SUMMARY"); then
  echo "rustc SEC017 SDK findings differ from validated baseline" >&2
  exit 1
fi

echo "rustc SEC017 SDK findings match validated baseline"
